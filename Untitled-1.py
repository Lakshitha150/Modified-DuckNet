...
model_path = 'ModelSaveTensorFlow/model' 
...
def load_data(img_height, img_width, images_to_be_loaded, dataset):
    ...
    print(f'Found {len(train_ids)} images in {IMAGES_PATH}')
    
    if len(train_ids) == 0:
        raise ValueError(f"No images found in {IMAGES_PATH}. Check the path and file extensions.")
...
if len(X) == 0 or len(Y) == 0:
    raise ValueError("The dataset is empty. Ensure that the dataset path and file extensions are correct.")
...
if len(X) > 1 and len(Y) > 1:
    ...
    raise ValueError("Not enough samples to split into train, validation, and test sets.")
...
print(f"Augmented images shape: {image_augmented.shape}, Augmented masks shape: {mask_augmented.shape}")
...
model = tf.keras.models.load_model(model_path + ".keras", custom_objects={'dice_metric_loss': dice_metric_loss})
